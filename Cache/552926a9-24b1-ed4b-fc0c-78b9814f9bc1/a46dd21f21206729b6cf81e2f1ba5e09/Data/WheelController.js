...
function ApplyMovementAndSteering(forwardTrigger, reverseTrigger, steerValue) {
    ...
    if (Math.abs(accelerationInput) > 0.0) {
        ...
        if (currentSpeed < script.maxSpeed || (accelerationInput > 0 && currentVelocity.dot(script.car.getTransform().left) < 0) || (accelerationInput < 0 && currentVelocity.dot(script.car.getTransform().left) > 0)) {
            var forwardForce = script.car.getTransform().left.uniformScale(accelerationInput * script.accelerationForce);
    ...
    if (Math.abs(steerValue) > 0.0) {
        // Try a much larger torque value and use absolute torque instead of relative
        var torqueMultiplier = 10.0; // Increase torque significantly
        var turningTorque = new vec3(0.0, steerValue * script.turnTorque * torqueMultiplier, 0.0);
        
        // Debug logging
        print("Applying torque: " + turningTorque.toString() + " with steer value: " + steerValue);
        
        // Try both relative and absolute torque
        script.car.addTorque(turningTorque, Physics.ForceMode.Force);
        
        // Alternative: Apply lateral force at front/back of car for turning
        var lateralForce = script.car.getTransform().right.uniformScale(steerValue * script.accelerationForce * 0.5);
        var frontOffset = script.car.getTransform().forward.uniformScale(2.0); // Offset from center
        script.car.addForceAtPosition(lateralForce, script.car.getTransform().getWorldPosition().add(frontOffset), Physics.ForceMode.Force);
    }
}

// Legacy function - no longer used in arcade mode
function AddTorque(triggerValue) {   
    // This function is kept for backward compatibility but not used in arcade mode
    print("AddTorque called - this function is deprecated in arcade mode");
}