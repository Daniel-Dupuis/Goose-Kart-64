...
function ApplyMovementAndSteering(forwardTrigger, reverseTrigger, steerValue) {
    ...
    // Apply steering using hinge motor velocity targets on Y-axis
    if (script.frontWheelConstraints && script.frontWheelConstraints.length >= 2) {
        var targetSteerVelocity = steerValue * script.maxSteerAngle * 0.1; // Convert to angular velocity
        
        // Apply steering velocity to front wheel hinge motors
        for (var i = 0; i < script.frontWheelConstraints.length; i++) {
            ...
            if (constraintComponent) {
                ...
                // Set the target velocity for the motor (Y-axis rotation)
                motorSettings.targetValue = targetSteerVelocity;
