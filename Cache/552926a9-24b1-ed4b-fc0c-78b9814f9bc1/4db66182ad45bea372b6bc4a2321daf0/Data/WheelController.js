...
onStartEvent.bind(function() {
    ...
    if (script.frontWheelConstraints) {
        for (var i = 0; i < script.frontWheelConstraints.length; i++) {
            ...
            motorSettings.targetValue = 0.0; // Start at center
...
function ApplyMovementAndSteering(forwardTrigger, reverseTrigger, steerValue) {
    ...
    // Apply torque to all wheels for movement (X-axis for wheel spinning)
    for (var i = 0; i < script.wheelBodys.length; i++) {
        script.wheelBodys[i].addRelativeTorque(new vec3(netTorque, 0.0, 0.0), Physics.ForceMode.Acceleration);
    }
...
function AddTorque(triggerValue) {   
    ...
    for (var i = 0; i<script.wheelBodys.length; i++) {
        script.wheelBodys[i].addRelativeTorque(new vec3(scaledForce, 0.0, 0.0), Physics.ForceMode.Acceleration);
    }
}