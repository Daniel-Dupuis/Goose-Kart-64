...
function ApplyMovementAndSteering(forwardTrigger, reverseTrigger, steerValue) {
    ...
    // Apply smooth steering by controlling hinge constraints
    if (script.frontWheelHinges && script.frontWheelHinges.length >= 2) {
        var targetSteerAngle = steerValue * script.maxSteerAngle; // Target angle in degrees
        var deltaTime = getDeltaTime();
        
        // Apply smooth steering to front wheel hinge constraints
        for (var i = 0; i < script.frontWheelHinges.length; i++) {
            var hinge = script.frontWheelHinges[i];
            if (hinge && i < currentSteerAngles.length) {
                ...
                // Set the hinge constraint target angle
                var targetAngleRadians = currentSteerAngles[i] * Math.PI / 180.0;
                hinge.setTargetAngle(targetAngleRadians);
            }
        }
    }
}
