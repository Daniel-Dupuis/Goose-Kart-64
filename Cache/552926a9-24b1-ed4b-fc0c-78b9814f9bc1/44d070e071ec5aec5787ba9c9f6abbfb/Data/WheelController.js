...
function ApplyMovementAndSteering(forwardTrigger, reverseTrigger, steerValue) {
    if (!script.carBody) {
        print("ERROR: carBody is not assigned!");
        return;
    }
    
    // Debug logging
    if (forwardTrigger > 0 || reverseTrigger > 0 || Math.abs(steerValue) > 0) {
        print("Input - Forward: " + forwardTrigger + ", Reverse: " + reverseTrigger + ", Steer: " + steerValue);
    }
    ...
    if (Math.abs(accelerationInput) > 0.0) {
        ...
        var currentVelocity = script.carBody.velocity;
        var currentSpeed = currentVelocity.length;
        
        print("Current speed: " + currentSpeed + ", Max speed: " + script.maxSpeed);
        
        // Only apply force if under max speed or trying to slow down
        if (currentSpeed < script.maxSpeed || (accelerationInput > 0 && currentVelocity.dot(script.carBody.getTransform().forward) < 0) || (accelerationInput < 0 && currentVelocity.dot(script.carBody.getTransform().forward) > 0)) {
            var forwardForce = script.carBody.getTransform().forward.uniformScale(accelerationInput * script.accelerationForce);
            print("Applying force: " + forwardForce.toString() + " with magnitude: " + forwardForce.length);
            script.carBody.addForce(forwardForce, Physics.ForceMode.Force);
        } else {
            print("Speed limit reached, not applying force");
        }
    }
    ...
    if (Math.abs(steerValue) > 0.0) {
        ...
        print("Applying torque: " + turningTorque.toString());
        script.carBody.addRelativeTorque(turningTorque, Physics.ForceMode.Force);
    }
}
