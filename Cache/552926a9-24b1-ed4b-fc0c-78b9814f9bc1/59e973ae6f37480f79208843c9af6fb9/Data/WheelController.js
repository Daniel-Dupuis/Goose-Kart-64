...
function ApplyMovementAndSteering(forwardTrigger, reverseTrigger, steerValue) {
    ...
    if (Math.abs(accelerationInput) > 0.0) {
        ...
        if (currentSpeed < script.maxSpeed || (accelerationInput > 0 && currentVelocity.dot(script.car.getTransform().left) < 0) || (accelerationInput < 0 && currentVelocity.dot(script.car.getTransform().forward) > 0)) {
