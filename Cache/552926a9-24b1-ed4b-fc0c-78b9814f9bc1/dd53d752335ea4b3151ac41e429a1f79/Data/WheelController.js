...
function ApplyMovementAndSteering(forwardTrigger, reverseTrigger, steerValue) {
    ...
    print("WheelController: Car mass: " + script.car.mass + ", Dynamic: " + script.car.dynamic + ", Intangible: " + script.car.intangible);
    print("WheelController: Car velocity: " + script.car.velocity.toString() + ", Angular velocity: " + script.car.angularVelocity.toString());
    ...
    if (Math.abs(accelerationInput) > 0.0) {
        ...
        if (currentSpeed < script.maxSpeed || (accelerationInput > 0 && currentVelocity.dot(script.car.getTransform().left) < 0) || (accelerationInput < 0 && currentVelocity.dot(script.car.getTransform().left) > 0)) {
            // Try using forward direction instead of left
            ...
            // Try both Force and Impulse modes
            script.car.addForce(forwardForce, Physics.ForceMode.Impulse);
            
            // Also try setting velocity directly as a test
            if (Math.abs(accelerationInput) > 0.5) {
                var testVelocity = script.car.getTransform().forward.uniformScale(accelerationInput * 10);
                print("WheelController: Setting test velocity: " + testVelocity.toString());
                script.car.velocity = testVelocity;
            }
        } else {
