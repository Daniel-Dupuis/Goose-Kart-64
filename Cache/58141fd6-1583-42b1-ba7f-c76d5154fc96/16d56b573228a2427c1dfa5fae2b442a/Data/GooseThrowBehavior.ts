...
export class GooseThrowBehavior extends BaseScriptComponent {
    ...
    onAwake() {
        print(`[GooseThrow] onAwake on '${this.sceneObject.name}'`);
        // Components
    ...
    private onUpdate() {
        ...
        if (handVelocity) {
            const mag = handVelocity.length ?? handVelocity.length();
            if (mag !== undefined) {
        ...
        if (this.isHolding && getDeltaTime() > 0) {
            ...
            const accMag = this.accumulatedForce.length ?? this.accumulatedForce.length();
