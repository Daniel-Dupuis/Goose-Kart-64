...
function initialize() {
    ...
    // Setup BLE game controller for reset functionality
...
function onSurfaceReset(eventData) {
    print("onSurfaceReset called! Original position: " + originalPosition);
    print("Current position before reset: " + script.getSceneObject().getTransform().getLocalPosition());
    script.getSceneObject().getTransform().setLocalPosition(originalPosition);
    print("Position after reset: " + script.getSceneObject().getTransform().getLocalPosition());
    setTrackingTarget();
    print("onSurfaceReset completed");
}
...
var lastLTPressed = false;
var debugCounter = 0;
function onUpdateEvent(eventData) {
    print("WorldObjectController onUpdateEvent called - frame " + debugCounter);
    
    // Check LT trigger for reset functionality
    ...
    if (debugCounter % 60 === 0) {
        if (buttonState) {
            print("WorldObjectController: ButtonState exists, LT value: " + (buttonState.lt || "undefined"));
        } else {
            print("WorldObjectController: ButtonState is null/undefined");
        }
    }
    
    if (buttonState) {
        // Use LT (left trigger) for reset - triggers range from 0.0 to 1.0
        var ltValue = buttonState.lt || 0.0;
        var isLTPressed = ltValue > 0.5; // Threshold for trigger press
        
        // Debug: Log when LT is pressed
        if (ltValue > 0.1) {
            print("WorldObjectController: LT detected with value: " + ltValue + ", threshold met: " + isLTPressed);
        }
        
        // Debug edge detection
        if (ltValue > 0.1) {
            print("Edge detection debug - isLTPressed: " + isLTPressed + ", lastLTPressed: " + lastLTPressed + ", will trigger: " + (isLTPressed && !lastLTPressed));
        }
        
        // Trigger reset on press (not hold)
        if (isLTPressed && !lastLTPressed) {
            print("CALLING onSurfaceReset() now!");
            onSurfaceReset();
            print("Surface reset triggered by LT trigger (value: " + ltValue + ")");
        }
        lastLTPressed = isLTPressed;
    }
