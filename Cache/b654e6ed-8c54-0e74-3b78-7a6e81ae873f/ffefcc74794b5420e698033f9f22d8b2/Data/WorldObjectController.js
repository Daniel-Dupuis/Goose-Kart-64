...
function initialize() {
    ...
    // bind events
    // Removed WorldTrackingResetEvent to disable touch-to-reset, using LT trigger instead
...
function onSurfaceReset(eventData) {
    print("Surface reset triggered by something");
...
function onUpdateEvent(eventData) {
    ...
    if (buttonState) {
        ...
        // Debug LT values
        if (ltValue > 0.1) {
            print("LT Debug - Value: " + ltValue + ", isPressed: " + isLTPressed + ", lastPressed: " + lastLTPressed);
        }
        
        // Trigger reset on press (not hold)
        if (isLTPressed && !lastLTPressed) {
            print("LT EDGE DETECTED - calling onSurfaceReset()");
            onSurfaceReset();
            print("Surface reset triggered by LT trigger");
        }
        lastLTPressed = isLTPressed;
    } else {
        print("No buttonState available");
    }
