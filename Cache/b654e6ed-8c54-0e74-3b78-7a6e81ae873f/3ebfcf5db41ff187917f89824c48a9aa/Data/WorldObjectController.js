...
function onSurfaceReset(eventData) {
    ...
    // Clear physics velocities for all child objects with BodyComponents
    var sceneObject = script.getSceneObject();
    clearPhysicsVelocities(sceneObject);
    
    setTrackingTarget();
}

function clearPhysicsVelocities(obj) {
    // Clear velocities for this object
    var bodyComponent = obj.getComponent("Physics.BodyComponent");
    if (bodyComponent) {
        bodyComponent.clearMotion();
    }
    
    // Recursively clear velocities for all children
    for (var i = 0; i < obj.getChildrenCount(); i++) {
        clearPhysicsVelocities(obj.getChild(i));
    }
}
...
function onUpdateEvent(eventData) {
    ...
    if (buttonState) {
        ...
        if (isLTPressed && !lastLTPressed) {
            onSurfaceReset();
            print("Surface reset triggered by LT trigger");
        }
